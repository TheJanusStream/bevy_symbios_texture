...
The `Genotype` implementations, the record-boundary envelope clamp and the
egui editor widgets are all generated from **one** table — `symbios-texture`'s
`for_each_texture_field!`, which carries a row per field of every config: its
kind, its envelope, the step mutation takes, and its UI label.  Each consumer
is a callback macro supplying only the semantics it owns; this crate's is
`impl_config_editor!` in [`src/ui.rs`](src/ui.rs), which maps a field kind to a
widget and its debouncing and knows nothing about any individual field.

A slider's range is therefore the same number the genetic search is clamped to
and the same number an application uses to bound a config arriving from an
untrusted peer.  Rows that need it carry two more columns: `explore` for a
field the search should stay inside a narrower band of than a person may
author, and a `layout` list where a panel's display order differs from the
table's — the table is in mutation-draw order, which is behaviour rather than
presentation.
